...
class EnvVarGuard {
  public:
    explicit EnvVarGuard(const std::vector<std::string> &names)
    {
        for (const auto &name : names) {
            const char *value = std::getenv(name.c_str());
            entries.push_back({name, value != nullptr, value != nullptr ? value : ""});
        }
    }
...
class PathOnlyUnlinkListener : public DiskCacheEvictListener {
  public:
    explicit PathOnlyUnlinkListener(std::unordered_map<std::string, uint64_t> &entries) : entries(entries) {}
...
{
    EnvVarGuard envGuard({"USER", "POD_IP", "BRPC_PORT", "STORAGE_THRESHOLD", "PARENT_PATH_LEVEL"});
...
TEST_F(DiskCacheUT, EvictNotifiesListenerWithStoredPath)
{
    ...
    DiskCache cache;
    ASSERT_EQ(cache.Start(cacheRoot, 1, 0.000001, 0.000001), 0);
...
TEST_F(DiskCacheUT, EvictAfterRenameWithoutPathRefreshUsesStalePath)
{
    std::string cacheRoot = "/tmp/testdir_evict_stale_path";
    std::filesystem::remove_all(cacheRoot);
    std::filesystem::create_directories(cacheRoot + "/0");
    SetRootPath(cacheRoot);
    SetTotalDirectory(1);
    ...
    uint64_t key = 702;
    std::string oldPath = "/logical/old-name";
    std::string newPath = "/logical/new-name";
    {
        std::ofstream out(GetFilePath(key));
        out << "renamed-cache";
    }
    ...
    RecordingEvictListener listener;
    cache.SetEvictListener(&listener);
    cache.InsertAndUpdate(key, 13, false, oldPath);

    // Simulate metadata rename oldPath -> newPath without refreshing DiskCache.
    cache.Evict(UINT64_MAX / 4);
    cache.SetEvictListener(nullptr);

    ASSERT_EQ(listener.items.size(), 1U);
    EXPECT_EQ(listener.items[0].inode, key);
    EXPECT_EQ(listener.items[0].path, oldPath);
    EXPECT_NE(listener.items[0].path, newPath);
    EXPECT_FALSE(std::filesystem::exists(GetFilePath(key)));

    std::filesystem::remove_all(cacheRoot);
}

TEST_F(DiskCacheUT, EvictAfterRenamePathRefreshDeletesRenamedPath)
{
    std::string cacheRoot = "/tmp/testdir_evict_renamed_path";
    std::filesystem::remove_all(cacheRoot);
    std::filesystem::create_directories(cacheRoot + "/0");
    SetRootPath(cacheRoot);
    SetTotalDirectory(1);
    ...
    metadataEntries[newPath] = inode;

    PathOnlyUnlinkListener listener(metadataEntries);
    cache.SetEvictListener(&listener);
    cache.InsertAndUpdate(inode, 12, false, oldPath);
    EXPECT_TRUE(cache.UpdatePath(inode, newPath));

    cache.Evict(UINT64_MAX / 4);
    cache.SetEvictListener(nullptr);

    EXPECT_EQ(listener.deletedPath, newPath);
    EXPECT_EQ(listener.deletedInode, inode);
    EXPECT_EQ(metadataEntries.count(oldPath), 0U);
    EXPECT_EQ(metadataEntries.count(newPath), 0U);
    EXPECT_FALSE(std::filesystem::exists(GetFilePath(inode)));

    std::filesystem::remove_all(cacheRoot);
}

TEST_F(DiskCacheUT, EvictAfterRenamePathRefreshDoesNotDeleteNewFileAtOldPath)
{
    std::string cacheRoot = "/tmp/testdir_evict_stale_path_recreate";
    std::filesystem::remove_all(cacheRoot);
    std::filesystem::create_directories(cacheRoot + "/0");
    SetRootPath(cacheRoot);
    SetTotalDirectory(1);
    ...
    uint64_t evictedInode = 801;
    uint64_t recreatedInode = 802;
    std::string oldPath = "/logical/old-name";
    std::string newPath = "/logical/new-name";
    {
    ...
    std::unordered_map<std::string, uint64_t> metadataEntries;
    metadataEntries[newPath] = evictedInode;
    metadataEntries[oldPath] = recreatedInode;

    PathOnlyUnlinkListener listener(metadataEntries);
    cache.SetEvictListener(&listener);
    cache.InsertAndUpdate(evictedInode, 12, false, oldPath);
    EXPECT_TRUE(cache.UpdatePath(evictedInode, newPath));

    cache.Evict(UINT64_MAX / 4);
    cache.SetEvictListener(nullptr);

    EXPECT_EQ(listener.deletedPath, newPath);
    EXPECT_EQ(listener.deletedInode, evictedInode);
    ASSERT_EQ(metadataEntries.count(oldPath), 1U);
    EXPECT_EQ(metadataEntries[oldPath], recreatedInode);
    EXPECT_EQ(metadataEntries.count(newPath), 0U);
...
TEST_F(DiskCacheUT, DiskCacheEvictPerformanceManual)
{
    const char *enabled = std::getenv("RUN_DISKCACHE_EVICT_PERF");
    if (enabled == nullptr || std::string(enabled) != "1") {
        GTEST_SKIP() << "set RUN_DISKCACHE_EVICT_PERF=1 to run manual evict performance test";
    }

    uint64_t fileCount = 5000;
    if (const char *value = std::getenv("DISKCACHE_EVICT_PERF_FILES")) {
        fileCount = std::stoull(value);
    }
    uint64_t fileSize = 4096;
    if (const char *value = std::getenv("DISKCACHE_EVICT_PERF_FILE_SIZE")) {
        fileSize = std::stoull(value);
    }

    std::string cacheRoot = "/tmp/testdir_evict_perf_manual";
    std::filesystem::remove_all(cacheRoot);
    std::filesystem::create_directories(cacheRoot + "/0");
    SetRootPath(cacheRoot);
    SetTotalDirectory(1);
    ...
    DiskCache cache;
    ASSERT_EQ(cache.Start(cacheRoot, 1, 0.000001, 0.0), 0);

    std::string payload(fileSize, 'x');
    for (uint64_t i = 0; i < fileCount; ++i) {
        uint64_t inode = 1000000 + i;
        {
            std::ofstream out(GetFilePath(inode), std::ios::binary);
            out << payload;
        }
        cache.InsertAndUpdate(inode, fileSize, false, "/logical/perf/" + std::to_string(inode));
    }
    ...
    uint64_t remainingFiles = 0;
    for (const auto &entry : std::filesystem::directory_iterator(cacheRoot + "/0")) {
        if (entry.is_regular_file()) {
            ++remainingFiles;
        }
    }

    std::cout << "DISKCACHE_EVICT_PERF_RESULT files=" << fileCount << " file_size=" << fileSize
              << " evicted=" << listener.items.size() << " remaining=" << remainingFiles
              << " elapsed_sec=" << elapsedSec << " files_per_sec=" << fileCount / elapsedSec
              << " mib_per_sec=" << (fileCount * fileSize / 1024.0 / 1024.0) / elapsedSec << std::endl;

    EXPECT_EQ(listener.items.size(), fileCount);
    EXPECT_EQ(remainingFiles, 0U);
