...
TEST(MetadbCoverageUT, SerializedHelperRoundTripFlow)
{
    /*
     * DT 对应关系:
     * - 补充 TC-DIR 目录类、TC-FILE 文件类、TC-REN rename 类、TC-KV KV 类、
     *   TC-SLICE slice 类用例所依赖的 serialized metadb 路径。
     * - 覆盖 TC-DIR-001 创建一级目录成功、
     *   TC-FILE-001 CREATE 成功并可见, TC-DIR-006 删除空目录成功,
     *   TC-FILE-006 UNLINK 删除后不可见、TC-REN-001 同目录文件重命名成功涉及的
     *   子操作参数编码/解码。
     * - 在不依赖服务状态的前提下，覆盖 create/stat/open/unlink/readdir/opendir
     *   响应编码，以及 rename 子操作成功/失败载荷的编码/解码。
     */
    ...
    // TC-DIR-001 / TC-FILE-001 / TC-DIR-006 / TC-FILE-006 / TC-REN-001:
    // 覆盖目录、文件、删除、rename 子操作参数的编码/解码。
    ...
    // TC-DIR/TC-FILE/TC-REN/TC-ATTR: 覆盖常用元数据响应编码路径。
    ...
    // TC-SLICE-004/005: 覆盖 slice put/get/delete 响应编码。
